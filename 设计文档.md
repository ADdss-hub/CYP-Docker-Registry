# CYP-Registry 完整开发设计文档（终极整合版 - 安全增强）
...
### 1.1 项目定位
CYP-Registry 是一款**零信任架构**的企业级容器镜像私有仓库管理系统，专为个人开发者及小型团队设计。系统深度融合供应链安全、智能加速、P2P 分发、镜像签名、SBOM 生成、环境感知、自适应配置等能力，实现开箱即用、全平台支持、零配置优化的极致体验，并采用**强制登录认证**和**异常自动锁定**机制确保系统绝对安全。
...
kubernetes:
  ...
  enable_probe: true
  probe_path: "/healthz"
  config_map_name: "cyp-registry-config"
  secret_name: "cyp-registry-secret"
...
```bash
# 1. 查看锁定状态
docker exec cyp-registry /app/cyp-registry cli lock status

# 2. 手动解锁
docker exec -it cyp-registry /app/scripts/unlock.sh

# 3. 查看审计日志
docker exec cyp-registry /app/cyp-registry cli audit tail -n 100

# 4. 导出审计日志
docker exec cyp-registry /app/cyp-registry cli audit export --start "2026-01-01" --end "2026-01-31" > audit.json

# 5. 验证日志完整性
docker exec cyp-registry /app/cyp-registry cli audit verify
```
...
# 方案2: 进入单用户模式
docker exec -it cyp-registry /app/cyp-registry server --single-user-mode

# 方案3: 手动删除锁定状态（危险操作）
docker exec cyp-registry sqlite3 /app/data/db.sqlite3 \
  "UPDATE system_status SET is_locked=0, lock_reason=''"

# 方案4: 使用恢复密钥（如果已配置）
docker exec cyp-registry /app/cyp-registry cli unlock --recovery-key "$RECOVERY_KEY"
```

**问题2: 误触发锁定**
```bash
# 1. 查看触发规则
docker logs cyp-registry | grep "Intrusion detected"
...
- **安全公告**: https://github.com/CYP/registry/security
- **文档**: https://docs.cyp-registry.com
- **讨论**: https://forum.cyp-registry.com
- **邮件**: nasDSSCYP@outlook.com
- **安全报告**: security@CYP-Registry.com
